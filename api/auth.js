...
export default async function handler(request, response) {
  ...
  if (url.pathname.endsWith('/auth/callback')) {
    ...
    try {
      ...
      if (!hasRequiredRole) {
        ...
        return response.redirect(302, `/login.html?error=${encodeURIComponent('No eres miembro de la LSAF. Para acceder al sistema debes ser un miembro activo. Mantente pendiente del canal de reclutamiento para más información.')}`);
      }
  ...
  console.error('Ruta no encontrada:', url.pathname);
